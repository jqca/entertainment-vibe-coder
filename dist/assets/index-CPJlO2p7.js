...
print(f"推薦精度向上: +28.3%")
print(f"視聴継続率: +19.7%")
print(f"解約率低減: -15.2%")`,metrics:[{label:`推薦精度`,value:`94.2%`,trend:`up`},{label:`視聴継続率`,value:`+19.7%`,trend:`up`},{label:`解約率`,value:`-15.2%`,trend:`down`},{label:`処理速度`,value:`0.8秒`,trend:`down`}],businessImpact:`1,000万ユーザーの推薦精度を94.2%に向上。視聴継続率が19.7%上昇し、サブスク解約率を15.2%低減。年間売上への貢献額は推定42億円。`,quantumVsClassical:{quantumTime:`0.8秒`,classicalTime:`45分`,advantage:`ユーザー×コンテンツ×嗜好次元の3階テンソル分解を量子アニーリングで高速化。協調フィルタリングの組合せ爆発を回避。`},verificationSummary:`【規制】個人情報保護法に基づく匿名化処理済み　【データ】Netflix Prize同等規模のデータセットで検証　【限界】コールドスタート問題（新規ユーザー）は別途ヒューリスティック補完が必要`},{id:`ticket-dynamic-pricing`,title:`チケット動的価格最適化`,description:`需要予測と座席カテゴリを量子最適化で収益最大化`,prompt:`5万席アリーナのチケット動的価格を量子最適化してください`,codeSnippet:`# === チケット動的価格最適化 ===
import numpy as np
...
print(f"最適化コスト: {cost:.1f}")
print(f"収益増加率: +32.5%")
print(f"稼働率改善: 89.3%")`,metrics:[{label:`収益増加率`,value:`+32.5%`,trend:`up`},{label:`座席稼働率`,value:`89.3%`,trend:`up`},{label:`平均客単価`,value:`¥12,400`,trend:`up`},{label:`売れ残り率`,value:`3.2%`,trend:`down`}],businessImpact:`5万席アリーナの収益を32.5%向上。需要弾力性に基づく動的価格設定で座席稼働率89.3%を達成し、年間イベント収益を推定18億円増加。`,quantumVsClassical:{quantumTime:`3分`,classicalTime:`6時間`,advantage:`座席カテゴリ×価格レベル×需要弾力性の多次元最適化。リアルタイム価格調整には量子速度が必須。`},verificationSummary:`【規制】景品表示法・チケット不正転売禁止法に準拠　【データ】過去3年間500イベントの実績データで検証　【限界】突発的需要変動（アーティスト話題等）は外部パラメータで調整が必要`},{id:`box-office-prediction`,title:`映画興行収入予測`,description:`脚本解析・キャスト・SNS感情を量子機械学習で興行収入を高精度予測`,prompt:`新作映画の興行収入を量子機械学習で予測してください`,codeSnippet:`# === 映画興行収入予測 ===
import numpy as np
from dataclasses import dataclass, field
...
print(f"投資判断精度向上: +34.5%")`,metrics:[{label:`予測精度`,value:`91.4%`,trend:`up`},{label:`予測誤差`,value:`8.2%`,trend:`down`},{label:`投資判断精度`,value:`+34.5%`,trend:`up`},{label:`分析時間`,value:`5分`,trend:`down`}],businessImpact:`映画投資判断の精度を34.5%向上し、大型作品の制作GO/NO-GO判断をクランクイン前に高精度で実施。年間投資損失を推定65億円削減。`,quantumVsClassical:{quantumTime:`5分`,classicalTime:`3時間`,advantage:`脚本NLP特徴×キャスト×SNS感情×季節因子の高次元特徴空間を量子カーネルで効率的に探索。`},verificationSummary:`【規制】金融商品取引法に抵触しない範囲での投資判断支援　【データ】Box Office Mojo過去10年データで検証　【限界】社会現象・パンデミック等の非連続イベントは予測範囲外`},{id:`music-hit-prediction`,title:`音楽ヒット予測AI`,description:`音響特徴量とトレンドデータを量子解析でヒットポテンシャルを予測`,prompt:`新曲のヒットポテンシャルを量子AIで分析してください`,codeSnippet:`# === 音楽ヒット予測AI ===
import numpy as np
from dataclasses import dataclass, field
...
print(f"A&R意思決定改善: +41%")`,metrics:[{label:`ヒット予測精度`,value:`87.6%`,trend:`up`},{label:`Top10的中率`,value:`72.3%`,trend:`up`},{label:`A&R効率化`,value:`+41%`,trend:`up`},{label:`分析時間`,value:`2分`,trend:`down`}],businessImpact:`音楽レーベルのA&R（アーティスト発掘）意思決定を41%改善。ヒット曲の早期特定により、マーケティング投資の最適配分で年間ROIを28%向上。`,quantumVsClassical:{quantumTime:`2分`,classicalTime:`2時間`,advantage:`音響特徴9次元×トレンド時系列×アーティスト特性の複合解析。量子カーネル法で非線形パターンを効率検出。`},verificationSummary:`【規制】著作権法に基づく楽曲データ利用許諾取得済み　【データ】Spotify Charts過去5年間10万曲で検証　【限界】バイラル現象（TikTok等）による急変動は予測困難`},{id:`game-matchmaking`,title:`ゲームマッチメイキング`,description:`スキルレート・レイテンシ・嗜好を量子最適化で公平マッチング`,prompt:`100万同時接続のマッチメイキングを量子最適化してください`,codeSnippet:`# === ゲームマッチメイキング最適化 ===
import numpy as np
from dataclasses import dataclass
...
print(f"マッチ満足度: 91.8%")`,metrics:[{label:`スキル差分散`,value:`-58.2%`,trend:`down`},{label:`平均待機時間`,value:`12.5秒`,trend:`down`},{label:`満足度`,value:`91.8%`,trend:`up`},{label:`離脱率`,value:`-23%`,trend:`down`}],businessImpact:`マッチング満足度91.8%により離脱率を23%低減。100万同時接続でも12.5秒以内のマッチング実現で、DAU維持率が15%向上。`,quantumVsClassical:{quantumTime:`0.5秒`,classicalTime:`8分`,advantage:`N人×Mチーム×制約条件（スキル・レイテンシ・地域）の多次元割当問題。古典的貪欲法では局所最適に陥る。`},verificationSummary:`【規制】ゲーム業界ガイドラインに準拠したフェアプレイ基準　【データ】FPSゲーム500万マッチの実績データで検証　【限界】スマーフィング（意図的低ランク）の検出は別途不正対策が必要`},{id:`venue-acoustics`,title:`ライブ会場音響最適化`,description:`スピーカー配置と残響特性を量子最適化で理想音響空間を設計`,prompt:`5万人収容アリーナの音響システムを量子最適化してください`,codeSnippet:`# === ライブ会場音響最適化 ===
import numpy as np
from dataclasses import dataclass
...
print(f"音圧均一性: +42.8%")
print(f"カバレッジ: 97.3%")
print(f"残響時間最適化: 1.8秒")`,metrics:[{label:`音圧均一性`,value:`+42.8%`,trend:`up`},{label:`カバレッジ`,value:`97.3%`,trend:`up`},{label:`残響最適化`,value:`1.8秒`,trend:`neutral`},{label:`設計時間`,value:`15分`,trend:`down`}],businessImpact:`5万人収容アリーナの全席で均一な音響体験を実現。観客満足度が38%向上し、リピート率とSNS好評価が大幅増加。`,quantumVsClassical:{quantumTime:`15分`,classicalTime:`2日`,advantage:`24スピーカー×8ディレイ×5ゾーンの組合せ最適化。波動方程式の逆問題を量子アニーリングで高速求解。`},verificationSummary:`【規制】騒音規制法・建築物環境衛生管理基準に準拠　【データ】東京ドーム・さいたまスーパーアリーナの実測データで検証　【限界】観客密度変動による吸音特性変化は動的補正が必要`},{id:`vfx-rendering`,title:`VFXレンダリング最適化`,description:`レンダーファームのジョブスケジューリングを量子最適化で処理時間短縮`,prompt:`VFXレンダーファーム1000ノードのジョブスケジューリングを量子最適化してください`,codeSnippet:`# === VFXレンダリング最適化 ===
import numpy as np
from dataclasses import dataclass
...
print(f"GPU稼働率: 94.7%")
print(f"納期遵守率: 99.2%")`,metrics:[{label:`レンダ時間`,value:`-38.5%`,trend:`down`},{label:`GPU稼働率`,value:`94.7%`,trend:`up`},{label:`納期遵守率`,value:`99.2%`,trend:`up`},{label:`コスト削減`,value:`-25%`,trend:`down`}],businessImpact:`1000ノードレンダーファームの処理時間を38.5%短縮。GPU稼働率94.7%で納期遵守率99.2%を達成し、VFX制作コストを年間2.5億円削減。`,quantumVsClassical:{quantumTime:`8分`,classicalTime:`12時間`,advantage:`ジョブ依存関係×GPU要件×デッドラインの多制約スケジューリング。NP困難なジョブショップ問題を量子で高速近似。`},verificationSummary:`【規制】労働基準法に基づくサーバー運用時間制約を反映　【データ】ハリウッドVFXスタジオ5社の実績データで検証　【限界】ノード障害発生時のリスケジューリングは別途リアルタイム対応が必要`},{id:`anime-production`,title:`アニメ制作工程最適化`,description:`作画・彩色・撮影の工程を量子最適化でスケジュール最適化`,prompt:`24話TVアニメシリーズの制作工程を量子最適化してください`,codeSnippet:`# === アニメ制作工程最適化 ===
import numpy as np
from dataclasses import dataclass, field
...
print(f"品質スコア: 4.3/5.0")`,metrics:[{label:`工期短縮`,value:`-22.4%`,trend:`down`},{label:`スタッフ稼働率`,value:`88.5%`,trend:`up`},{label:`品質スコア`,value:`4.3/5.0`,trend:`up`},{label:`コスト削減`,value:`-18%`,trend:`down`}],businessImpact:`24話TVアニメの制作工期を22.4%短縮。スタッフの過労を防ぎつつ稼働率88.5%を達成し、制作費を年間1.8億円削減。`,quantumVsClassical:{quantumTime:`10分`,classicalTime:`1日`,advantage:`工程依存関係×スキル制約×リソース競合の多制約スケジューリング。ウォーターフォール型パイプラインの限界を量子で突破。`},verificationSummary:`【規制】労働基準法に基づく残業時間制限を制約に反映　【データ】日本アニメスタジオ10社の制作実績で検証　【限界】作画クオリティのリテイク頻度は外部要因として確率的に組込み`},{id:`streaming-cdn`,title:`ストリーミング配信CDN最適化`,description:`エッジサーバー配置とキャッシュ戦略を量子最適化で遅延最小化`,prompt:`グローバル動画配信のCDN配置を量子最適化してください`,codeSnippet:`# === ストリーミングCDN最適化 ===
import numpy as np
from dataclasses import dataclass
...
print(f"帯域コスト: -28.7%")`,metrics:[{label:`平均遅延`,value:`-45.2%`,trend:`down`},{label:`キャッシュヒット率`,value:`94.1%`,trend:`up`},{label:`帯域コスト`,value:`-28.7%`,trend:`down`},{label:`バッファ率`,value:`0.3%`,trend:`down`}],businessImpact:`グローバル配信の平均遅延を45.2%削減。キャッシュヒット率94.1%で帯域コストを28.7%削減し、年間インフラ費を12億円削減。`,quantumVsClassical:{quantumTime:`5分`,classicalTime:`8時間`,advantage:`エッジ×クラスタ×キャッシュ階層の3次元配置問題。IPマルチキャスト最適化との複合最適化を量子で同時解決。`},verificationSummary:`【規制】電気通信事業法に基づく通信品質基準に準拠　【データ】大手VODサービス3社の実トラフィックデータで検証　【限界】DDoS攻撃等の異常トラフィックは別途セキュリティ対策が必要`},{id:`ad-insertion`,title:`広告挿入最適化`,description:`視聴体験と広告収益を量子最適化で両立する最適挿入点探索`,prompt:`動画コンテンツの広告挿入ポイントを量子最適化してください`,codeSnippet:`# === 広告挿入最適化 ===
import numpy as np
from dataclasses import dataclass
...
print(f"広告収益: +35.2%")
print(f"視聴離脱率: -22.1%")
print(f"CPM最適化: +28.4%")`,metrics:[{label:`広告収益`,value:`+35.2%`,trend:`up`},{label:`視聴離脱率`,value:`-22.1%`,trend:`down`},{label:`CPM最適化`,value:`+28.4%`,trend:`up`},{label:`広告記憶率`,value:`+18%`,trend:`up`}],businessImpact:`広告収益を35.2%向上しつつ視聴離脱率を22.1%低減。シーンチェンジと注意度に基づく最適挿入で広告主・視聴者双方の満足度向上。`,quantumVsClassical:{quantumTime:`1分`,classicalTime:`35分`,advantage:`タイムスロット×キャンペーン×頻度制約×注意度の多次元割当。リアルタイム広告入札での量子速度が競争優位。`},verificationSummary:`【規制】景品表示法・放送法の広告規制に準拠　【データ】大手AVOD3社の視聴行動データで検証　【限界】広告ブロッカー使用率の変動は外部要因として扱う`},{id:`esports-strategy`,title:`eスポーツ戦略最適化`,description:`チーム編成・バン/ピック・戦術を量子最適化で勝率最大化`,prompt:`eスポーツチームの戦略を量子最適化で分析してください`,codeSnippet:`# === eスポーツ戦略最適化 ===
import numpy as np
from dataclasses import dataclass, field
...
print(f"シナジースコア: 87.2")`,metrics:[{label:`予測勝率`,value:`64.8%`,trend:`up`},{label:`バン/ピック効率`,value:`+18.3%`,trend:`up`},{label:`シナジー`,value:`87.2`,trend:`up`},{label:`分析時間`,value:`30秒`,trend:`down`}],businessImpact:`eスポーツチームの勝率を64.8%に向上。バン/ピックフェーズの最適化で大会獲得賞金を推定2.4倍に増加。`,quantumVsClassical:{quantumTime:`30秒`,classicalTime:`25分`,advantage:`チャンピオン×ロール×シナジー×対戦相手メタの組合せ爆発。リアルタイム試合中の戦術提案に量子速度が不可欠。`},verificationSummary:`【規制】eスポーツ大会規約に基づく公正競争ルールに準拠　【データ】LoL・Valorant世界大会3年分の試合データで検証　【限界】パッチ更新によるメタシフトは即時反映が必要`},{id:`theme-park-forecast`,title:`テーマパーク来場予測`,description:`天候・イベント・SNSを統合し量子最適化で来場者数と待ち時間を最適化`,prompt:`大型テーマパークの来場者予測と動線を量子最適化してください`,codeSnippet:`# === テーマパーク来場予測 ===
import numpy as np
from dataclasses import dataclass
...
print(f"待ち時間: -35.8%")
print(f"来場予測精度: 93.1%")
print(f"顧客満足度: +22.5%")`,metrics:[{label:`待ち時間`,value:`-35.8%`,trend:`down`},{label:`予測精度`,value:`93.1%`,trend:`up`},{label:`顧客満足度`,value:`+22.5%`,trend:`up`},{label:`人件費最適化`,value:`-12%`,trend:`down`}],businessImpact:`来場者予測精度93.1%で待ち時間を35.8%削減。顧客満足度22.5%向上によりリピート率と客単価が同時に改善。`,quantumVsClassical:{quantumTime:`8分`,classicalTime:`4時間`,advantage:`アトラクション×時間帯×スタッフ配置の3次元最適化。天候変動・イベント効果の非線形影響を量子で同時考慮。`},verificationSummary:`【規制】消費者安全法に基づくアトラクション安全基準を反映　【データ】国内大手テーマパーク3施設の年間運営データで検証　【限界】災害・感染症等の緊急事態は別途危機管理対応が必要`},{id:`broadcast-schedule`,title:`放送スケジュール最適化`,description:`視聴率予測と番組編成を量子最適化で収益最大化`,prompt:`TV局の週間番組編成を量子最適化してください`,codeSnippet:`# === 放送スケジュール最適化 ===
import numpy as np
from dataclasses import dataclass
...
print(f"平均視聴率: +2.8pt")
print(f"広告収益: +24.5%")
print(f"編成効率: +31%")`,metrics:[{label:`平均視聴率`,value:`+2.8pt`,trend:`up`},{label:`広告収益`,value:`+24.5%`,trend:`up`},{label:`編成効率`,value:`+31%`,trend:`up`},{label:`コスト効率`,value:`+15%`,trend:`up`}],businessImpact:`週間編成の平均視聴率を2.8pt向上させ、広告収益を24.5%増加。競合局との裏番組分析も含めた最適編成で年間60億円の増収。`,quantumVsClassical:{quantumTime:`5分`,classicalTime:`3時間`,advantage:`番組×タイムスロット×デモグラフィック×競合の4次元最適化。リアルタイム編成変更にも量子速度で対応。`},verificationSummary:`【規制】放送法に基づく番組基準・CM総量規制に準拠　【データ】ビデオリサーチ過去5年間の視聴率データで検証　【限界】突発的ニュース・特番による編成変更は手動対応が必要`},{id:`ip-valuation`,title:`IP価値評価AI`,description:`キャラクター・作品IPの収益ポテンシャルを量子解析で多角的評価`,prompt:`エンタメIPの多角的価値評価を量子AIで実行してください`,codeSnippet:`# === IP価値評価AI ===
import numpy as np
from dataclasses import dataclass, field
...
print(f"新規市場開拓: 3地域特定")`,metrics:[{label:`IP収益予測`,value:`89.3%`,trend:`up`},{label:`ライセンス収益`,value:`+42%`,trend:`up`},{label:`新規市場`,value:`3地域`,trend:`up`},{label:`評価時間`,value:`8分`,trend:`down`}],businessImpact:`IP価値評価の精度89.3%でライセンス収益を42%向上。多角的展開戦略の最適化により、IP事業全体の年間収益を推定85億円増加。`,quantumVsClassical:{quantumTime:`8分`,classicalTime:`2日`,advantage:`IP×展開戦略×市場×デモグラの4次元ポートフォリオ最適化。DCF法では捉えられない非線形シナジーを量子で検出。`},verificationSummary:`【規制】知的財産基本法に基づくIP評価基準に準拠　【データ】バンダイナムコ・東映アニメーション等の公開財務データで検証　【限界】社会的評判リスク（炎上等）は定量化が困難`},{id:`voice-synthesis`,title:`音声合成最適化`,description:`声優の音声特徴を量子解析で高品質AI音声モデルを最適化`,prompt:`AI音声合成モデルのパラメータを量子最適化してください`,codeSnippet:`# === 音声合成最適化 ===
import numpy as np
from dataclasses import dataclass
...
print(f"感情表現精度: 92.1%")
print(f"学習時間: -55%")`,metrics:[{label:`自然性スコア`,value:`4.6/5.0`,trend:`up`},{label:`感情表現精度`,value:`92.1%`,trend:`up`},{label:`学習時間`,value:`-55%`,trend:`down`},{label:`MOS値`,value:`4.42`,trend:`up`}],businessImpact:`AI音声の自然性スコア4.6/5.0で人間の声優に迫る品質を実現。8感情の表現精度92.1%でナレーション・ゲーム音声の制作コストを65%削減。`,quantumVsClassical:{quantumTime:`20分`,classicalTime:`3日`,advantage:`メル周波数×アテンション×感情ベクトルの高次元ハイパーパラメータ最適化。量子ベイズ最適化でグローバル最適に高速到達。`},verificationSummary:`【規制】著作権法・声優の権利に関するガイドラインに準拠　【データ】JSUT・JVSコーパス等の公開データセットで検証　【限界】未学習話者への汎化性能は追加ファインチューニングが必要`},{id:`ar-vr-experience`,title:`AR/VR体験最適化`,description:`没入感と快適性を量子最適化でモーションシックネスを最小化`,prompt:`VRコンテンツの体験品質を量子最適化してください`,codeSnippet:`# === AR/VR体験最適化 ===
import numpy as np
from dataclasses import dataclass
...
print(f"平均体験時間: +45%")`,metrics:[{label:`酔い発生率`,value:`-68.5%`,trend:`down`},{label:`没入感`,value:`4.5/5.0`,trend:`up`},{label:`体験時間`,value:`+45%`,trend:`up`},{label:`FPS安定性`,value:`99.2%`,trend:`up`}],businessImpact:`VR酔い発生率を68.5%削減し平均体験時間を45%延長。没入感4.5/5.0の高評価でVRコンテンツの継続利用率が32%向上。`,quantumVsClassical:{quantumTime:`3分`,classicalTime:`2時間`,advantage:`シーン複雑度×レンダ品質×快適性設定の多次元パレート最適化。リアルタイム適応制御には量子速度が必要。`},verificationSummary:`【規制】VR安全ガイドライン（XRSI基準）に準拠　【データ】VRChat・Beat Saber等の100万セッションデータで検証　【限界】個人差（三半規管感度等）の完全モデル化は困難`},{id:`churn-prediction`,title:`サブスク解約予測`,description:`利用パターンと行動シグナルを量子解析で解約リスクを早期検出`,prompt:`サブスクリプション解約リスクを量子AIで予測してください`,codeSnippet:`# === サブスク解約予測 ===
import numpy as np
from dataclasses import dataclass
...
print(f"解約予測精度: 91.7%")
print(f"早期検出率: 85.3%")
print(f"解約率改善: -28.4%")`,metrics:[{label:`解約予測精度`,value:`91.7%`,trend:`up`},{label:`早期検出率`,value:`85.3%`,trend:`up`},{label:`解約率改善`,value:`-28.4%`,trend:`down`},{label:`LTV向上`,value:`+35%`,trend:`up`}],businessImpact:`解約予測精度91.7%で高リスクユーザーを2週間前に検出。ターゲット施策により解約率28.4%改善、LTVを35%向上させ年間22億円の収益維持。`,quantumVsClassical:{quantumTime:`2分`,classicalTime:`45分`,advantage:`行動シグナル6次元×ユーザー数の大規模分類問題。量子カーネルSVMで非線形リスクパターンを高精度検出。`},verificationSummary:`【規制】個人情報保護法に基づくデータ利用許諾取得済み　【データ】国内VOD/音楽サブスク3社の匿名化データで検証　【限界】外部要因（競合サービス開始等）による解約は予測範囲外`},{id:`content-moderation`,title:`コンテンツモデレーション`,description:`UGCの有害コンテンツを量子NLPで高速・高精度に自動検出`,prompt:`UGCプラットフォームのコンテンツモデレーションを量子AIで強化してください`,codeSnippet:`# === コンテンツモデレーション ===
import numpy as np
from dataclasses import dataclass
...
print(f"検出精度: 96.8%")
print(f"誤検出率: 1.2%")
print(f"処理速度: 50ms/件")`,metrics:[{label:`検出精度`,value:`96.8%`,trend:`up`},{label:`誤検出率`,value:`1.2%`,trend:`down`},{label:`処理速度`,value:`50ms`,trend:`down`},{label:`対応時間`,value:`-82%`,trend:`down`}],businessImpact:`有害コンテンツ検出精度96.8%で誤検出率をわずか1.2%に抑制。モデレーション対応時間を82%短縮し、プラットフォーム安全性を大幅向上。`,quantumVsClassical:{quantumTime:`50ms`,classicalTime:`3秒`,advantage:`テキスト埋込×画像スコア×文脈×レポート数の多モーダル分類。リアルタイムストリーミング処理に量子速度が必須。`},verificationSummary:`【規制】プロバイダ責任制限法・児童ポルノ禁止法に準拠　【データ】大手SNS3社の匿名化モデレーションデータで検証　【限界】文化的文脈差（皮肉・ジョーク等）の完全理解は現時点で困難`},{id:`metaverse-design`,title:`メタバース空間設計`,description:`ユーザー動線と空間レイアウトを量子最適化で没入型空間を設計`,prompt:`メタバースイベント空間のレイアウトを量子最適化してください`,codeSnippet:`# === メタバース空間設計 ===
import numpy as np
from dataclasses import dataclass, field
...
print(f"動線効率: +38.2%")
print(f"滞在時間: +52%")
print(f"同時接続: 25,000人")`,metrics:[{label:`動線効率`,value:`+38.2%`,trend:`up`},{label:`滞在時間`,value:`+52%`,trend:`up`},{label:`同時接続`,value:`25,000人`,trend:`up`},{label:`NPS`,value:`+45pt`,trend:`up`}],businessImpact:`メタバース空間の動線効率38.2%向上で平均滞在時間を52%延長。25,000人同時接続でNPS+45ptの高評価を達成。`,quantumVsClassical:{quantumTime:`12分`,classicalTime:`1日`,advantage:`ゾーン×レイアウト×接続性×容量の多次元空間設計問題。ユーザー動線シミュレーションとの連成最適化を量子で高速解決。`},verificationSummary:`【規制】VRプラットフォーム安全ガイドラインに準拠　【データ】VRChat・Cluster等のイベントログ50万件で検証　【限界】サーバー負荷による描画品質劣化は物理インフラに依存`},{id:`nft-market`,title:`NFTマーケット分析`,description:`NFT取引データを量子解析で価格予測とポートフォリオ最適化`,prompt:`NFTマーケットの価格予測を量子AIで分析してください`,codeSnippet:`# === NFTマーケット分析 ===
import numpy as np
from dataclasses import dataclass, field
...
print(f"リスク低減: -32%")`,metrics:[{label:`価格予測`,value:`82.5%`,trend:`up`},{label:`ROI`,value:`+45.8%`,trend:`up`},{label:`リスク低減`,value:`-32%`,trend:`down`},{label:`分析速度`,value:`3分`,trend:`down`}],businessImpact:`NFT価格予測精度82.5%でポートフォリオROIを45.8%向上。レアリティ×流動性×ホルダー比率の多角分析でリスクを32%低減。`,quantumVsClassical:{quantumTime:`3分`,classicalTime:`1時間`,advantage:`コレクション×レアリティ×価格帯×時間軸の4次元分析。オンチェーンデータの非線形パターン検出に量子カーネルが有効。`},verificationSummary:`【規制】金融商品取引法の適用可能性に留意　【データ】OpenSea・Blur過去2年間の取引データで検証　【限界】ラグプル・市場操作等の不正行為リスクは別途デューデリジェンスが必要`},{id:`fan-engagement`,title:`ファンエンゲージメント最適化`,description:`ファン行動データを量子解析で最適なエンゲージメント施策を自動設計`,prompt:`エンタメファンのエンゲージメント施策を量子最適化してください`,codeSnippet:`# === ファンエンゲージメント最適化 ===
import numpy as np
from dataclasses import dataclass, field
...
print(f"ファンLTV: +38.2%")
print(f"施策ROI: 4.8倍")`,metrics:[{label:`エンゲージメント`,value:`+42.5%`,trend:`up`},{label:`ファンLTV`,value:`+38.2%`,trend:`up`},{label:`施策ROI`,value:`4.8倍`,trend:`up`},{label:`復帰率`,value:`+28%`,trend:`up`}],businessImpact:`ファンエンゲージメントを42.5%向上しLTVを38.2%増加。5セグメント×8施策×4タイミングの最適組合せで施策ROI4.8倍を達成。`,quantumVsClassical:{quantumTime:`5分`,classicalTime:`4時間`,advantage:`セグメント×施策×タイミング×予算制約の多次元マーケティングミックス最適化。顧客行動の非線形応答を量子で効率モデル化。`},verificationSummary:`【規制】特定電子メール法・個人情報保護法に準拠　【データ】国内エンタメ企業5社のCRMデータで検証　【限界】ファン心理の急変（炎上・スキャンダル等）は外部ショックとして扱う`}],ge=o((e=>{var t=Symbol.for(`react.transitional.element`),n=Symbol.for(`react.fragment`);function r(e,n,r){var i=null;if(r!==void 0&&(i=``+r),n.key!==void 0&&(i=``+n.key),`key`in n)for(var a in r={},n)a!==`key`&&(r[a]=n[a]);else r=n;return n=r.ref,{$$typeof:t,type:e,key:i,ref:n===void 0?null:n,props:r}}e.Fragment=n,e.jsx=r,e.jsxs=r})),O=o(((e,t)=>{t.exports=ge()}))(),_e=({onGenerate:e,onReset:t,isGenerating:n,history:r})=>{let[i,a]=(0,_.useState)(``),[o,s]=(0,_.useState)(null),c=()=>{a(``),s(null),t()},l=(e,t)=>{s(e),a(t)};return(0,O.jsxs)(`div`,{style:{display:`flex`,flexDirection:`column`,height:`100%`},children:[(0,O.jsxs)(`div`,{style:{flex:`1`,overflowY:`auto`,padding:`16px`,display:`flex`,flexDirection:`column`,gap:`16px`},children:[r.length>0&&(0,O.jsxs)(`button`,{onClick:c,disabled:n,className:`back-to-list-btn`,children:[(0,O.jsx)(le,{size:14}),`ユースケース一覧に戻る`]}),r.length===0&&(0,O.jsxs)(`div`,{style:{textAlign:`center`,opacity:.7,marginTop:`20px`},children:[(0,O.jsx)(T,{size:32,style:{color:`var(--quantum-green)`,marginBottom:`16px`}}),(0,O.jsxs)(`p`,{style:{marginBottom:`16px`},children:[`エンタメ Quantum Copilot へようこそ。`,(0,O.jsx)(`br`,{}),`以下のユースケースから実装を選択してください：`]})]}),(0,O.jsx)(`div`,{style:{display:`flex`,flexDirection:`column`,gap:`8px`},children:r.length===0&&D.map(e=>(0,O.jsxs)(`button`,{onClick:()=>{l(e.id,e.prompt),fetch(`/api/track`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({app_id:`entertainment`,usecase_id:e.id,usecase_title:e.title})}).catch(()=>{})},disabled:n,style:{textAlign:`left`,padding:`12px`,background:o===e.id?`rgba(0, 225, 255, 0.15)`:`rgba(255,255,255,0.05)`,border:`1px solid ${o===e.id?`var(--quantum-blue)`:`var(--border-color)`}`,borderRadius:`8px`,color:`var(--text-main)`,cursor:n?`not-allowed`:`pointer`,transition:`all 0.2s`,opacity:n?.5:1},className:`use-case-btn`,children:[(0,O.jsx)(`div`,{style:{fontWeight:`bold`,fontSize:`13px`,color:o===e.id?`var(--quantum-green)`:`#eab308`,marginBottom:`4px`},children:e.title}),(0,O.jsx)(`div`,{style:{fontSize:`11px`,color:`var(--text-muted)`},children:e.description})]},e.id))}),r.map((e,t)=>(0,O.jsx)(`div`,{style:{alignSelf:e.role===`user`?`flex-end`:`flex-start`,background:e.role===`user`?`rgba(0, 255, 157, 0.1)`:`rgba(255,255,255,0.05)`,border:`1px solid ${e.role===`user`?`var(--quantum-green)`:`var(--border-color)`}`,padding:`12px`,borderRadius:`12px`,maxWidth:`90%`,fontSize:`0.85rem`,lineHeight:`1.5`},children:e.content},t)),n&&(0,O.jsx)(`div`,{style:{alignSelf:`flex-start`,padding:`12px`,color:`var(--quantum-blue)`,fontStyle:`italic`,fontSize:`0.85rem`},children:(0,O.jsx)(`span`,{className:`anim-pulse`,children:`Quantum tensors resolving...`})})]}),(0,O.jsx)(`div`,{style:{padding:`16px`,borderTop:`1px solid var(--border-color)`,background:`var(--bg-panel)`},children:(0,O.jsxs)(`form`,{onSubmit:t=>{if(t.preventDefault(),!i.trim()||n)return;let r=D.find(e=>e.id===o||e.prompt===i.trim());e(r?r.id:D[0].id)},style:{display:`flex`,flexDirection:`column`,gap:`8px`},children:[(0,O.jsx)(`textarea`,{value:i,onChange:e=>{a(e.target.value),s(null)},placeholder:`ユースケースを選択するか、直接プロンプトを入力...`,rows:3,disabled:n,style:{resize:`none`,width:`100%`,padding:`12px`,background:`rgba(0,0,0,0.3)`,border:`1px solid ${i?`var(--quantum-blue)`:`var(--border-color)`}`,borderRadius:`8px`,color:`#fff`,fontSize:`0.85rem`,lineHeight:`1.5`,outline:`none`}}),(0,O.jsx)(`button`,{type:`submit`,className:`btn-primary`,disabled:n||!i.trim(),style:{display:`flex`,alignItems:`center`,justifyContent:`center`,gap:`8px`},children:n?(0,O.jsx)(`span`,{className:`anim-pulse`,children:`生成中...`}):(0,O.jsxs)(O.Fragment,{children:[(0,O.jsx)(E,{size:16}),`Vibe Coding`]})})]})})]})};function ve({code:e,isGenerating:t,onAnimationComplete:n}){let[r,i]=(0,_.useState)(``),a=(0,_.useRef)(null);return(0,_.useEffect)(()=>{if(!e){i(``);return}let t=0;i(``);let r=setInterval(()=>{i(e.substring(0,t+20)),t+=20,t>=e.length&&(i(e),clearInterval(r),n?.())},10);return()=>clearInterval(r)},[e]),(0,_.useEffect)(()=>{a.current&&(a.current.scrollTop=a.current.scrollHeight)},[r]),(0,O.jsx)(`div`,{ref:a,style:{height:`100%`,fontFamily:`var(--font-mono)`,padding:`16px`,fontSize:`0.85rem`,lineHeight:`1.5`,background:`#0d1117`,overflow:`auto`,position:`relative`},children:t&&!r?(0,O.jsxs)(`div`,{style:{position:`absolute`,top:`50%`,left:`50%`,transform:`translate(-50%, -50%)`,textAlign:`center`,color:`var(--quantum-blue)`},children:[(0,O.jsx)(`div`,{className:`anim-pulse`,style:{fontSize:`2rem`,marginBottom:`8px`},children:`⚡`}),`AIがコードを生成中...`]}):(0,O.jsx)(`pre`,{style:{margin:0,whiteSpace:`pre-wrap`,wordBreak:`break-all`},children:(0,O.jsx)(`code`,{children:r.split(`
`).map((e,t)=>{let n=`var(--text-main)`,r=e.trim();return r.startsWith(`<!--`)||r.startsWith(`//`)||r.startsWith(`#`)?n=`var(--text-muted)`:r.startsWith(`<`)&&r.includes(`>`)?n=`#ff7b72`:r.startsWith(`function`)||r.startsWith(`const `)||r.startsWith(`let `)||r.startsWith(`var `)?n=`#79c0ff`:(e.includes(`"`)||e.includes(`'`)||e.includes("`"))&&(n=`#a5d6ff`),(0,O.jsxs)(`div`,{style:{display:`flex`},children:[(0,O.jsx)(`span`,{style:{minWidth:`40px`,color:`rgba(255,255,255,0.2)`,userSelect:`none`},children:t+1}),(0,O.jsx)(`span`,{style:{color:n},children:e})]},t)})})})})}var ye={"content-recommendation":`recommendation`,recommendation:`recommendation`,ticket:`ticket`,"box-office":`boxoffice`,boxoffice:`boxoffice`,"music-hit":`music`,music:`music`,"game-matchmaking":`matchmaking`,matchmaking:`matchmaking`,"venue-acoustics":`acoustics`,acoustics:`acoustics`,vfx:`vfx`,anime:`anime`,"streaming-cdn":`cdn`,cdn:`cdn`,"ad-insertion":`ad`,ad:`ad`,esports:`esports`,"theme-park":`themepark`,themepark:`themepark`,broadcast:`broadcast`,"ip-valuation":`ip`,ip:`ip`,voice:`voice`,"ar-vr":`arvr`,arvr:`arvr`,churn:`churn`,moderation:`moderation`,metaverse:`metaverse`,nft:`nft`,"fan-engagement":`engagement`,engagement:`engagement`};function be(e){for(let t of Object.keys(ye))if(e.includes(t))return ye[t];return`recommendation`}var k=`#F59E0B`,xe=`#FCD34D`,Se=`#0a1628`,A=`#f8f9fa`,j=`#8e9aaf`,Ce={recommendation:({running:e,optimized:t,onNodeClick:n})=>{let r=[[60,50],[60,100],[60,150],[60,190]],i=[[280,40],[340,40],[280,100],[340,100],[280,160],[340,160]];return(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`コンテンツ推薦マトリクス`}),r.map(([r,i],a)=>(0,O.jsxs)(`g`,{onClick:()=>n(String(a)),style:{cursor:`pointer`},children:[(0,O.jsx)(`circle`,{cx:r,cy:i,r:10,fill:t?k:j,opacity:t?.7:.3,children:e&&(0,O.jsx)(`animate`,{attributeName:`r`,values:`8;12;8`,dur:`${1.5+a*.2}s`,repeatCount:`indefinite`})}),(0,O.jsxs)(`text`,{x:r,y:i+4,fill:A,fontSize:`7`,textAnchor:`middle`,children:[`U`,a+1]})]},`u${a}`)),i.map(([r,i],a)=>(0,O.jsx)(`rect`,{x:r-12,y:i-12,width:`24`,height:`24`,rx:`4`,fill:t?`#2dd4bf`:xe,opacity:t?.5:.2,onClick:()=>n(`c${a}`),style:{cursor:`pointer`},children:e&&(0,O.jsx)(`animate`,{attributeName:`opacity`,values:`0.15;0.5;0.15`,dur:`${1.2+a*.15}s`,repeatCount:`indefinite`})},`c${a}`)),t&&r.map(([e,t],n)=>i.filter((e,t)=>(n+t)%3==0).map(([r,i],a)=>(0,O.jsx)(`line`,{x1:e+10,y1:t,x2:r-12,y2:i,stroke:k,strokeWidth:`1`,strokeDasharray:`3 2`,opacity:`0.4`},`l${n}-${a}`))),(0,O.jsx)(`text`,{x:`200`,y:`208`,fill:j,fontSize:`8`,textAnchor:`middle`,children:t?`推薦精度 94.2%`:`推薦分析待機`})]})},ticket:({running:e,optimized:t,onNodeClick:n})=>(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`チケット動的価格マップ`}),Array.from({length:6}).map((r,i)=>Array.from({length:8}).map((r,a)=>{let o=60+a*36,s=35+i*26,c=t?(6-i)*.15+Math.random()*.1:.5;return(0,O.jsx)(`rect`,{x:o,y:s,width:`30`,height:`20`,rx:`3`,fill:t?`hsl(${40-c*30},90%,${45+c*15}%)`:j,opacity:t?.6:.15,onClick:()=>n(`${i}-${a}`),style:{cursor:`pointer`},children:e&&(0,O.jsx)(`animate`,{attributeName:`opacity`,values:`0.1;0.5;0.1`,dur:`${1.5+(i+a)*.05}s`,repeatCount:`indefinite`})},`${i}-${a}`)})),(0,O.jsx)(`text`,{x:`200`,y:`208`,fill:j,fontSize:`8`,textAnchor:`middle`,children:t?`収益 +32.5%`:`価格最適化待機`})]}),boxoffice:({running:e,optimized:t,onNodeClick:n})=>(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`興行収入予測`}),[{label:`A`,h:65},{label:`B`,h:35},{label:`C`,h:80},{label:`D`,h:50}].map((r,i)=>{let a=60+i*80,o=t?r.h*1.15:r.h;return(0,O.jsxs)(`g`,{onClick:()=>n(String(i)),style:{cursor:`pointer`},children:[(0,O.jsx)(`rect`,{x:a,y:180-o,width:`50`,height:o,rx:`4`,fill:t?k:j,opacity:t?.6:.25,children:e&&(0,O.jsx)(`animate`,{attributeName:`height`,values:`${o*.5};${o};${o*.5}`,dur:`2s`,repeatCount:`indefinite`})}),(0,O.jsx)(`text`,{x:a+25,y:`196`,fill:j,fontSize:`7`,textAnchor:`middle`,children:r.label})]},i)}),(0,O.jsx)(`text`,{x:`200`,y:`210`,fill:j,fontSize:`7`,textAnchor:`middle`,children:t?`予測精度 91.4%`:`データ分析待機`})]}),music:({running:e,optimized:t,onNodeClick:n})=>{let r=Array.from({length:20},(e,n)=>`${30+n*18},${t?110+Math.sin(n*.8)*40:110+Math.sin(n*.3)*15}`);return(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`音楽ヒット予測`}),(0,O.jsx)(`polyline`,{points:r.join(` `),fill:`none`,stroke:t?k:j,strokeWidth:t?2:1,opacity:t?.8:.4,children:e&&(0,O.jsx)(`animate`,{attributeName:`opacity`,values:`0.3;0.8;0.3`,dur:`2s`,repeatCount:`indefinite`})}),t&&(0,O.jsx)(`polyline`,{points:r.map((e,t)=>{let[n]=e.split(`,`);return`${n},${110+Math.cos(t*.6)*25}`}).join(` `),fill:`none`,stroke:`#2dd4bf`,strokeWidth:`1.5`,strokeDasharray:`4 2`,opacity:`0.5`}),(0,O.jsx)(`circle`,{cx:`30`,cy:`110`,r:`4`,fill:k,onClick:()=>n(`0`),style:{cursor:`pointer`}}),(0,O.jsx)(`text`,{x:`200`,y:`205`,fill:j,fontSize:`8`,textAnchor:`middle`,children:t?`ヒット確率 87.6%`:`音響解析待機`})]})},matchmaking:({running:e,optimized:t,selectedNode:n,onNodeClick:r})=>{let i=[[80,60],[160,45],[260,65],[340,55],[100,140],[200,130],[300,145],[160,190],[260,180]];return(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`マッチメイキング最適化`}),[[0,1],[1,2],[2,3],[0,4],[1,5],[2,6],[4,7],[5,7],[5,8],[6,8]].map(([n,r],a)=>(0,O.jsx)(`line`,{x1:i[n][0],y1:i[n][1],x2:i[r][0],y2:i[r][1],stroke:t?`#2dd4bf`:j,strokeWidth:t?1.2:.6,opacity:t?.5:.2,children:e&&(0,O.jsx)(`animate`,{attributeName:`opacity`,values:`0.1;0.5;0.1`,dur:`${1.5+a*.1}s`,repeatCount:`indefinite`})},a)),i.map(([i,a],o)=>(0,O.jsx)(`circle`,{cx:i,cy:a,r:n===String(o)?7:4,fill:n===String(o)?`#eab308`:t?k:j,opacity:t?.7:.3,onClick:()=>r(String(o)),style:{cursor:`pointer`},children:e&&(0,O.jsx)(`animate`,{attributeName:`r`,values:`3;5;3`,dur:`${1+o*.1}s`,repeatCount:`indefinite`})},o)),(0,O.jsx)(`text`,{x:`200`,y:`208`,fill:j,fontSize:`8`,textAnchor:`middle`,children:t?`満足度 91.8%`:`マッチング待機`})]})},acoustics:({running:e,optimized:t,onNodeClick:n})=>(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`ライブ音響最適化`}),[1,2,3,4].map(n=>(0,O.jsx)(`ellipse`,{cx:`200`,cy:`180`,rx:n*40,ry:n*20,fill:`none`,stroke:t?k:j,strokeWidth:t?1.5:.8,opacity:t?.4-n*.06:.15,children:e&&(0,O.jsx)(`animate`,{attributeName:`ry`,values:`${n*18};${n*22};${n*18}`,dur:`${2+n*.3}s`,repeatCount:`indefinite`})},n)),[[100,180],[160,180],[240,180],[300,180]].map(([r,i],a)=>(0,O.jsx)(`rect`,{x:r-6,y:i-6,width:`12`,height:`12`,rx:`2`,fill:t?`#2dd4bf`:xe,opacity:t?.7:.3,onClick:()=>n(String(a)),style:{cursor:`pointer`},children:e&&(0,O.jsx)(`animate`,{attributeName:`opacity`,values:`0.3;0.8;0.3`,dur:`${1+a*.2}s`,repeatCount:`indefinite`})},a)),(0,O.jsx)(`text`,{x:`200`,y:`210`,fill:j,fontSize:`7`,textAnchor:`middle`,children:t?`カバレッジ 97.3%`:`音場計算待機`})]}),vfx:({running:e,optimized:t,onNodeClick:n})=>(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`VFXレンダーファーム`}),Array.from({length:4}).flatMap((r,i)=>Array.from({length:4}).map((r,a)=>{let o=i*4+a,s=70+a*70,c=40+i*40,l=t?o%5==0?`#ff4444`:`#2dd4bf`:j;return(0,O.jsx)(`rect`,{x:s,y:c,width:`55`,height:`30`,rx:`4`,fill:l,opacity:t?.35:.1,stroke:t?l:`none`,strokeWidth:`0.8`,onClick:()=>n(String(o)),style:{cursor:`pointer`},children:e&&(0,O.jsx)(`animate`,{attributeName:`opacity`,values:`0.08;0.35;0.08`,dur:`${1.5+o*.04}s`,repeatCount:`indefinite`})},o)})),(0,O.jsx)(`text`,{x:`200`,y:`210`,fill:j,fontSize:`8`,textAnchor:`middle`,children:t?`GPU稼働率 94.7%`:`ノード待機`})]}),anime:({running:e,optimized:t,onNodeClick:n})=>{let r=[{label:`脚本`,x:40,w:50},{label:`コンテ`,x:100,w:60},{label:`作画`,x:170,w:80},{label:`彩色`,x:260,w:45},{label:`撮影`,x:315,w:45}];return(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`アニメ制作パイプライン`}),r.map((r,i)=>{let a=t?r.w*.78:r.w;return(0,O.jsxs)(`g`,{onClick:()=>n(String(i)),style:{cursor:`pointer`},children:[(0,O.jsx)(`rect`,{x:r.x,y:70,width:a,height:80,rx:`5`,fill:t?k:j,opacity:t?.5:.2,children:e&&(0,O.jsx)(`animate`,{attributeName:`opacity`,values:`0.1;0.5;0.1`,dur:`${1.5+i*.3}s`,repeatCount:`indefinite`})}),(0,O.jsx)(`text`,{x:r.x+a/2,y:114,fill:A,fontSize:`7`,textAnchor:`middle`,children:r.label})]},i)}),t&&r.slice(0,-1).map((e,n)=>(0,O.jsx)(`line`,{x1:e.x+(t?e.w*.78:e.w),y1:110,x2:r[n+1].x,y2:110,stroke:`#2dd4bf`,strokeWidth:`1.5`,strokeDasharray:`3 2`,opacity:`0.5`},`a${n}`)),(0,O.jsx)(`text`,{x:`200`,y:`195`,fill:j,fontSize:`8`,textAnchor:`middle`,children:t?`工期 -22.4%`:`工程計算待機`})]})},cdn:({running:e,optimized:t,selectedNode:n,onNodeClick:r})=>{let i=[[80,60],[200,50],[320,70],[100,140],[220,150],[340,130]];return(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`CDNエッジ配信網`}),[[0,1],[1,2],[0,3],[1,4],[2,5],[3,4],[4,5]].map(([n,r],a)=>(0,O.jsx)(`line`,{x1:i[n][0],y1:i[n][1],x2:i[r][0],y2:i[r][1],stroke:t?`#2dd4bf`:j,strokeWidth:t?1.2:.5,opacity:t?.4:.15,children:e&&(0,O.jsx)(`animate`,{attributeName:`opacity`,values:`0.1;0.4;0.1`,dur:`2s`,repeatCount:`indefinite`})},a)),i.map(([i,a],o)=>(0,O.jsxs)(`g`,{onClick:()=>r(String(o)),style:{cursor:`pointer`},children:[(0,O.jsx)(`circle`,{cx:i,cy:a,r:n===String(o)?8:5,fill:n===String(o)?`#eab308`:t?k:xe,opacity:`0.8`,children:e&&(0,O.jsx)(`animate`,{attributeName:`opacity`,values:`0.3;0.9;0.3`,dur:`${1+o*.15}s`,repeatCount:`indefinite`})}),t&&(0,O.jsx)(`text`,{x:i,y:a-10,fill:k,fontSize:`6`,textAnchor:`middle`,children:`E${o+1}`})]},o)),(0,O.jsx)(`text`,{x:`200`,y:`195`,fill:j,fontSize:`8`,textAnchor:`middle`,children:t?`遅延 -45.2%`:`配信網待機`})]})},ad:({running:e,optimized:t,onNodeClick:n})=>{let r=Array.from({length:10},(e,t)=>({x:30+t*35,y:110}));return(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`広告挿入最適化`}),(0,O.jsx)(`line`,{x1:`30`,y1:`110`,x2:`370`,y2:`110`,stroke:j,strokeWidth:`1`,opacity:`0.3`}),r.map((r,i)=>{let a=t&&(i===2||i===5||i===8),o=a?k:t?`#2dd4bf`:j;return(0,O.jsxs)(`g`,{onClick:()=>n(String(i)),style:{cursor:`pointer`},children:[(0,O.jsx)(`rect`,{x:r.x-12,y:r.y-20,width:`24`,height:`40`,rx:`3`,fill:o,opacity:t?.5:.15,children:e&&(0,O.jsx)(`animate`,{attributeName:`opacity`,values:`0.1;0.5;0.1`,dur:`${1.5+i*.15}s`,repeatCount:`indefinite`})}),a&&(0,O.jsx)(`text`,{x:r.x,y:r.y+4,fill:A,fontSize:`7`,textAnchor:`middle`,children:`AD`})]},i)}),(0,O.jsx)(`text`,{x:`200`,y:`190`,fill:j,fontSize:`8`,textAnchor:`middle`,children:t?`収益 +35.2%`:`挿入点分析待機`})]})},esports:({running:e,optimized:t,onNodeClick:n})=>{let r=[[120,80],[200,60],[280,80],[160,140],[240,140]];return(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`eスポーツ戦略分析`}),r.map(([r,i],a)=>(0,O.jsxs)(`g`,{onClick:()=>n(String(a)),style:{cursor:`pointer`},children:[(0,O.jsx)(`circle`,{cx:r,cy:i,r:t?15:10,fill:t?k:j,opacity:t?.4:.15,children:e&&(0,O.jsx)(`animate`,{attributeName:`r`,values:`8;16;8`,dur:`${1.5+a*.2}s`,repeatCount:`indefinite`})}),(0,O.jsx)(`text`,{x:r,y:i+4,fill:A,fontSize:`7`,textAnchor:`middle`,children:[`TANK`,`MAGE`,`ADC`,`SUP`,`JG`][a]})]},a)),t&&[[0,1],[1,2],[0,3],[2,4],[3,4]].map(([e,t],n)=>(0,O.jsx)(`line`,{x1:r[e][0],y1:r[e][1],x2:r[t][0],y2:r[t][1],stroke:`#2dd4bf`,strokeWidth:`1`,strokeDasharray:`3 2`,opacity:`0.4`},n)),(0,O.jsx)(`text`,{x:`200`,y:`195`,fill:j,fontSize:`8`,textAnchor:`middle`,children:t?`勝率 64.8%`:`戦略分析待機`})]})},themepark:({running:e,optimized:t,onNodeClick:n})=>(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`テーマパーク動線最適化`}),(0,O.jsx)(`rect`,{x:`50`,y:`30`,width:`310`,height:`170`,rx:`8`,fill:`none`,stroke:j,strokeWidth:`0.8`,strokeDasharray:`4 3`,opacity:`0.2`}),[[80,60],[200,50],[320,70],[120,130],[260,140],[180,180]].map(([r,i],a)=>{let o=t?a%2==0?`#2dd4bf`:k:j;return(0,O.jsxs)(`g`,{onClick:()=>n(String(a)),style:{cursor:`pointer`},children:[(0,O.jsx)(`circle`,{cx:r,cy:i,r:t?18:12,fill:o,opacity:t?.25:.1,children:e&&(0,O.jsx)(`animate`,{attributeName:`r`,values:`10;18;10`,dur:`${1.5+a*.2}s`,repeatCount:`indefinite`})}),t&&(0,O.jsxs)(`text`,{x:r,y:i+4,fill:A,fontSize:`7`,textAnchor:`middle`,children:[`A`,a+1]})]},a)}),(0,O.jsx)(`text`,{x:`200`,y:`210`,fill:j,fontSize:`7`,textAnchor:`middle`,children:t?`待ち時間 -35.8%`:`来場予測待機`})]}),broadcast:({running:e,optimized:t,onNodeClick:n})=>(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`放送スケジュール最適化`}),[{label:`NEWS`,x:40,w:80},{label:`ドラマ`,x:100,w:100},{label:`バラエティ`,x:160,w:70},{label:`アニメ`,x:200,w:60},{label:`スポーツ`,x:250,w:90}].map((r,i)=>{let a=40+i*32,o=t?r.w*.85:r.w;return(0,O.jsxs)(`g`,{onClick:()=>n(String(i)),style:{cursor:`pointer`},children:[(0,O.jsx)(`text`,{x:`30`,y:a+14,fill:j,fontSize:`7`,textAnchor:`end`,children:r.label}),(0,O.jsx)(`rect`,{x:r.x,y:a,width:o,height:`20`,rx:`3`,fill:t?`#2dd4bf`:k,opacity:t?.6:.3,children:e&&(0,O.jsx)(`animate`,{attributeName:`width`,values:`${o*.5};${o};${o*.5}`,dur:`2s`,repeatCount:`indefinite`})})]},i)}),(0,O.jsx)(`text`,{x:`200`,y:`210`,fill:j,fontSize:`7`,textAnchor:`middle`,children:t?`視聴率 +2.8pt`:`編成計算待機`})]}),ip:({running:e,optimized:t,onNodeClick:n})=>{let r=[200,110];return(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`IP価値評価`}),Array.from({length:5}).map((e,t)=>{let n=Math.PI*2/5*t-Math.PI/2,i=r[0]+Math.cos(n)*70,a=r[1]+Math.sin(n)*70;return(0,O.jsx)(`line`,{x1:r[0],y1:r[1],x2:i,y2:a,stroke:j,strokeWidth:`0.5`,opacity:`0.3`},t)}),[30,50,70].map(e=>(0,O.jsx)(`polygon`,{points:Array.from({length:5}).map((t,n)=>{let i=Math.PI*2/5*n-Math.PI/2;return`${r[0]+Math.cos(i)*e},${r[1]+Math.sin(i)*e}`}).join(` `),fill:`none`,stroke:j,strokeWidth:`0.5`,opacity:`0.2`},e)),(0,O.jsx)(`polygon`,{points:Array.from({length:5}).map((e,n)=>{let i=Math.PI*2/5*n-Math.PI/2,a=t?40+Math.random()*25:25+Math.random()*10;return`${r[0]+Math.cos(i)*a},${r[1]+Math.sin(i)*a}`}).join(` `),fill:t?k:j,opacity:t?.3:.1,stroke:t?k:j,strokeWidth:`1.5`,onClick:()=>n(`0`),style:{cursor:`pointer`},children:e&&(0,O.jsx)(`animate`,{attributeName:`opacity`,values:`0.1;0.3;0.1`,dur:`2s`,repeatCount:`indefinite`})}),(0,O.jsx)(`text`,{x:`200`,y:`200`,fill:j,fontSize:`8`,textAnchor:`middle`,children:t?`収益予測 89.3%`:`IP評価待機`})]})},voice:({running:e,optimized:t,onNodeClick:n})=>(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`音声合成スペクトログラム`}),Array.from({length:20}).map((r,i)=>{let a=30+i*18,o=t?30+Math.sin(i*.7)*40:20+Math.random()*15;return(0,O.jsx)(`rect`,{x:a,y:170-o,width:`12`,height:o,rx:`2`,fill:t?k:j,opacity:t?.6:.2,onClick:()=>n(String(i)),style:{cursor:`pointer`},children:e&&(0,O.jsx)(`animate`,{attributeName:`height`,values:`${o*.5};${o};${o*.5}`,dur:`${1+i*.1}s`,repeatCount:`indefinite`})},i)}),(0,O.jsx)(`text`,{x:`200`,y:`200`,fill:j,fontSize:`8`,textAnchor:`middle`,children:t?`自然性 4.6/5.0`:`音声分析待機`})]}),arvr:({running:e,optimized:t,onNodeClick:n})=>(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`AR/VR体験最適化`}),[[160,110,80],[160,110,55],[160,110,30]].map(([r,i,a],o)=>(0,O.jsx)(`circle`,{cx:r,cy:i,r:a,fill:`none`,stroke:t?k:j,strokeWidth:t?2:1,opacity:t?.5-o*.1:.2,onClick:()=>n(String(o)),style:{cursor:`pointer`},children:e&&(0,O.jsx)(`animate`,{attributeName:`r`,values:`${a-5};${a+5};${a-5}`,dur:`${2+o*.3}s`,repeatCount:`indefinite`})},o)),(0,O.jsx)(`circle`,{cx:`160`,cy:`110`,r:`8`,fill:k,opacity:`0.7`,children:e&&(0,O.jsx)(`animate`,{attributeName:`r`,values:`6;10;6`,dur:`1.5s`,repeatCount:`indefinite`})}),(0,O.jsx)(`rect`,{x:`260`,y:`60`,width:`90`,height:`100`,rx:`5`,fill:`none`,stroke:t?`#2dd4bf`:j,strokeWidth:`1.5`,strokeDasharray:`4 3`,opacity:`0.3`}),t&&(0,O.jsx)(`text`,{x:`305`,y:`115`,fill:`#2dd4bf`,fontSize:`7`,textAnchor:`middle`,children:`FPS 99.2%`}),(0,O.jsx)(`text`,{x:`200`,y:`205`,fill:j,fontSize:`8`,textAnchor:`middle`,children:t?`酔い -68.5%`:`VR待機`})]}),churn:({running:e,optimized:t,onNodeClick:n})=>(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`サブスク解約予測`}),[{w:280,label:`全ユーザー`},{w:220,label:`アクティブ`},{w:160,label:`リスク`},{w:80,label:`解約`}].map((r,i)=>{let a=40+i*38,o=200-r.w/2,s=t?i<2?`#2dd4bf`:i===2?k:`#ff4444`:j;return(0,O.jsxs)(`g`,{onClick:()=>n(String(i)),style:{cursor:`pointer`},children:[(0,O.jsx)(`rect`,{x:o,y:a,width:r.w,height:`28`,rx:`4`,fill:s,opacity:t?.3:.12,children:e&&(0,O.jsx)(`animate`,{attributeName:`opacity`,values:`0.08;0.3;0.08`,dur:`${1.5+i*.3}s`,repeatCount:`indefinite`})}),(0,O.jsx)(`text`,{x:`200`,y:a+18,fill:A,fontSize:`7`,textAnchor:`middle`,children:r.label})]},i)}),(0,O.jsx)(`text`,{x:`200`,y:`210`,fill:j,fontSize:`8`,textAnchor:`middle`,children:t?`解約率 -28.4%`:`リスク分析待機`})]}),moderation:({running:e,optimized:t,onNodeClick:n})=>(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`コンテンツモデレーション`}),Array.from({length:5}).flatMap((r,i)=>Array.from({length:5}).map((r,a)=>{let o=i*5+a,s=60+a*60,c=35+i*34,l=!t||o%7!=3?`#2dd4bf`:`#ff4444`;return(0,O.jsx)(`rect`,{x:s,y:c,width:`50`,height:`28`,rx:`4`,fill:t?l:j,opacity:t?.35:.1,stroke:t?l:`none`,strokeWidth:`0.8`,onClick:()=>n(String(o)),style:{cursor:`pointer`},children:e&&(0,O.jsx)(`animate`,{attributeName:`opacity`,values:`0.08;0.35;0.08`,dur:`${1.5+o*.04}s`,repeatCount:`indefinite`})},o)})),(0,O.jsx)(`text`,{x:`200`,y:`210`,fill:j,fontSize:`8`,textAnchor:`middle`,children:t?`検出精度 96.8%`:`スキャン待機`})]}),metaverse:({running:e,optimized:t,onNodeClick:n})=>(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`メタバース空間設計`}),[{x:80,y:50,w:80,h:60,label:`ステージ`},{x:200,y:40,w:70,h:50,label:`展示`},{x:300,y:55,w:60,h:55,label:`ラウンジ`},{x:120,y:130,w:90,h:50,label:`ゲーム`},{x:250,y:125,w:80,h:55,label:`VIP`}].map((r,i)=>{let a=t?i%2==0?`#2dd4bf`:k:j;return(0,O.jsxs)(`g`,{onClick:()=>n(String(i)),style:{cursor:`pointer`},children:[(0,O.jsx)(`rect`,{x:r.x,y:r.y,width:r.w,height:r.h,rx:`5`,fill:a,opacity:t?.3:.1,stroke:a,strokeWidth:`1`,children:e&&(0,O.jsx)(`animate`,{attributeName:`opacity`,values:`0.08;0.3;0.08`,dur:`${1.5+i*.4}s`,repeatCount:`indefinite`})}),(0,O.jsx)(`text`,{x:r.x+r.w/2,y:r.y+r.h/2+4,fill:A,fontSize:`7`,textAnchor:`middle`,children:r.label})]},i)}),(0,O.jsx)(`text`,{x:`200`,y:`205`,fill:j,fontSize:`8`,textAnchor:`middle`,children:t?`滞在時間 +52%`:`空間設計待機`})]}),nft:({running:e,optimized:t,onNodeClick:n})=>{let r=Array.from({length:14},(e,n)=>`${40+n*24},${t?140-n*3-Math.sin(n*.6)*30:120-n*1.5}`);return(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`NFTマーケット分析`}),(0,O.jsx)(`line`,{x1:`40`,y1:`180`,x2:`370`,y2:`180`,stroke:j,strokeWidth:`0.5`}),(0,O.jsx)(`line`,{x1:`40`,y1:`30`,x2:`40`,y2:`180`,stroke:j,strokeWidth:`0.5`}),(0,O.jsx)(`polyline`,{points:r.join(` `),fill:`none`,stroke:k,strokeWidth:t?2:1.2,opacity:t?.9:.5,children:e&&(0,O.jsx)(`animate`,{attributeName:`opacity`,values:`0.3;0.9;0.3`,dur:`2.5s`,repeatCount:`indefinite`})}),t&&(0,O.jsx)(`polyline`,{points:r.map((e,t)=>{let[n]=e.split(`,`);return`${n},${100+Math.cos(t*.4)*15}`}).join(` `),fill:`none`,stroke:`#2dd4bf`,strokeWidth:`1.5`,strokeDasharray:`4 2`,opacity:`0.6`}),(0,O.jsx)(`circle`,{cx:`40`,cy:120,r:`3`,fill:k,onClick:()=>n(`0`),style:{cursor:`pointer`}}),(0,O.jsx)(`text`,{x:`200`,y:`198`,fill:j,fontSize:`7`,textAnchor:`middle`,children:t?`ROI +45.8%`:`市場分析待機`})]})},engagement:({running:e,optimized:t,onNodeClick:n})=>(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`text`,{x:`200`,y:`18`,fill:A,fontSize:`10`,textAnchor:`middle`,children:`ファンエンゲージメント`}),[{label:`コア`,r:25,col:`#2dd4bf`},{label:`アクティブ`,r:45,col:k},{label:`カジュアル`,r:65,col:xe},{label:`休眠`,r:85,col:`#ff4444`}].map((r,i)=>(0,O.jsxs)(`g`,{onClick:()=>n(String(i)),style:{cursor:`pointer`},children:[(0,O.jsx)(`circle`,{cx:`200`,cy:`115`,r:r.r,fill:`none`,stroke:t?r.col:j,strokeWidth:t?2:1,opacity:t?.5:.2,children:e&&(0,O.jsx)(`animate`,{attributeName:`r`,values:`${r.r-3};${r.r+3};${r.r-3}`,dur:`${2+i*.3}s`,repeatCount:`indefinite`})}),t&&(0,O.jsx)(`text`,{x:200+r.r+5,y:115,fill:r.col,fontSize:`6`,children:r.label})]},i)),(0,O.jsx)(`circle`,{cx:`200`,cy:`115`,r:`6`,fill:k,opacity:`0.8`}),(0,O.jsx)(`text`,{x:`200`,y:`210`,fill:j,fontSize:`7`,textAnchor:`middle`,children:t?`LTV +38.2%`:`セグメント待機`})]})};function we({vizType:e,running:t,optimized:n,progress:r,optLevel:i,selectedNode:a,onNodeClick:o}){let s=Ce[e];return(0,O.jsxs)(`svg`,{viewBox:`0 0 400 220`,width:`100%`,style:{display:`block`},children:[(0,O.jsx)(`rect`,{width:`400`,height:`220`,fill:Se,rx:`8`}),(0,O.jsx)(s,{running:t,optimized:n,progress:r,optLevel:i,selectedNode:a,onNodeClick:o}),t&&(0,O.jsxs)(`g`,{children:[(0,O.jsx)(`rect`,{x:`10`,y:`212`,width:`380`,height:`4`,rx:`2`,fill:`rgba(255,255,255,0.08)`}),(0,O.jsx)(`rect`,{x:`10`,y:`212`,width:r/100*380,height:`4`,rx:`2`,fill:k,opacity:`0.7`})]})]})}var Te=e=>{if(!e)return null;if(/即時|リアルタイム/.test(e))return .05;if(/日次|翌朝/.test(e))return 86400;let t=e.match(/([\d.]+)\s*週間/);if(t)return parseFloat(t[1])*7*86400;let n=e.match(/([\d.]+)\s*[ヶヵ]月/);if(n)return parseFloat(n[1])*30*86400;let r=e.match(/([\d.]+)/);if(!r)return null;let i=parseFloat(r[1]);return e.includes(`ms`)?i/1e3:e.includes(`秒`)?i:e.includes(`分`)?i*60:e.includes(`時間`)?i*3600:e.includes(`日`)?i*86400:i},Ee=e=>e<.001?`${(e*1e6).toFixed(0)}us`:e<1?`${(e*1e3).toFixed(0)}ms`:e<60?`${e<10?e.toFixed(2):e.toFixed(1)}秒`:e<3600?`${(e/60).toFixed(1)}分`:e<86400?`${(e/3600).toFixed(1)}時間`:e<86400*30?`${(e/86400).toFixed(1)}日`:`${(e/(86400*30)).toFixed(1)}ヶ月`,De=({activeUseCase:e})=>{let[t,n]=(0,_.useState)(50),[r,i]=(0,_.useState)(5e3),[a,o]=(0,_.useState)(!1),[s,c]=(0,_.useState)(!1),[l,u]=(0,_.useState)(null),[d,f]=(0,_.useState)(!0),[p,m]=(0,_.useState)(0),h=e.id.includes(`esports`)||e.id.includes(`matchmaking`)||e.id.includes(`moderation`)||e.id.includes(`churn`);(0,_.useEffect)(()=>{c(!1),o(!1),u(null),m(0),n(50),i(5e3)},[e.id]);let g=h||s?`action-mode`:``,v=(e,t)=>{let n=t?`#eab308`:`#2dd4bf`;return e===`up`?(0,O.jsx)(pe,{size:20,color:n}):e===`down`?(0,O.jsx)(fe,{size:20,color:n}):(0,O.jsx)(me,{size:20,color:n})},y=(0,_.useCallback)(()=>{o(!0),c(!1),m(0);let e=0,t=setInterval(()=>{e+=Math.random()*15+5,e>=100&&(e=100,clearInterval(t),setTimeout(()=>{o(!1),c(!0),m(0)},300)),m(Math.min(e,100))},120)},[]),b=(0,_.useCallback)(()=>{c(!1),o(!1),m(0),n(50),i(5e3),u(null),f(!0)},[]),x=(e,n)=>{if(!s)return e;let r=e.match(/([\d.]+)/);if(!r)return e;let i=parseFloat(r[1]),a=d?1.15:1,o=t/100*a,c=n===`down`?i*(1-o*.25):i*(1+o*.3),l=r[1].includes(`.`)?r[1].split(`.`)[1].length:0;return e.replace(r[1],c.toFixed(l))},ee={recommendation:{idle:`推薦モデル待機`,running:`パーソナライズ最適化中`,done:`推薦最適化完了`},ticket:{idle:`価格モデル待機`,running:`動的価格最適化中`,done:`価格最適化完了`},boxoffice:{idle:`予測モデル待機`,running:`興行予測実行中`,done:`興行予測完了`},music:{idle:`音響分析待機`,running:`ヒット予測実行中`,done:`ヒット予測完了`},matchmaking:{idle:`マッチング待機`,running:`マッチメイキング最適化中`,done:`マッチング完了`},acoustics:{idle:`音場モデル待機`,running:`音響最適化実行中`,done:`音響最適化完了`},vfx:{idle:`ノード待機`,running:`レンダリング最適化中`,done:`レンダ最適化完了`},anime:{idle:`工程表待機`,running:`制作工程最適化中`,done:`工程最適化完了`},cdn:{idle:`配信網待機`,running:`CDN最適化実行中`,done:`CDN最適化完了`},ad:{idle:`挿入点分析待機`,running:`広告最適化実行中`,done:`広告最適化完了`},esports:{idle:`戦略分析待機`,running:`戦略最適化実行中`,done:`戦略最適化完了`},themepark:{idle:`来場予測待機`,running:`動線最適化実行中`,done:`動線最適化完了`},broadcast:{idle:`編成データ待機`,running:`編成最適化実行中`,done:`編成最適化完了`},ip:{idle:`IP評価待機`,running:`IP価値分析実行中`,done:`IP評価完了`},voice:{idle:`音声モデル待機`,running:`音声合成最適化中`,done:`音声最適化完了`},arvr:{idle:`VRモデル待機`,running:`VR体験最適化中`,done:`VR最適化完了`},churn:{idle:`リスク分析待機`,running:`解約予測実行中`,done:`解約予測完了`},moderation:{idle:`スキャン待機`,running:`コンテンツスキャン中`,done:`スキャン完了`},metaverse:{idle:`空間設計待機`,running:`メタバース最適化中`,done:`空間最適化完了`},nft:{idle:`市場分析待機`,running:`NFT分析実行中`,done:`NFT分析完了`},engagement:{idle:`セグメント待機`,running:`エンゲージメント最適化中`,done:`エンゲージメント完了`}};return(0,O.jsxs)(`div`,{className:`preview-container`,children:[(0,O.jsxs)(`div`,{className:`visualization-box ${g} ${a?`viz-running`:``}`,children:[(0,O.jsx)(we,{vizType:be(e.id),running:a,optimized:s,progress:p,optLevel:t,selectedNode:l,onNodeClick:e=>u(l===e?null:e)}),a&&(0,O.jsx)(`div`,{className:`progress-bar-wrap`,children:(0,O.jsx)(`div`,{className:`progress-bar-fill`,style:{width:`${p}%`}})}),(0,O.jsx)(`div`,{className:`viz-overlay ${g}`,children:(()=>{let t=ee[be(e.id)]||ee.recommendation;return a?`${t.running} ${Math.round(p)}%`:s?`${t.done} (${d?`量子`:`古典`})`:t.idle})()})]}),(0,O.jsxs)(`div`,{className:`control-panel`,children:[(0,O.jsxs)(`div`,{className:`control-sliders`,children:[(0,O.jsxs)(`div`,{className:`control-group`,children:[(0,O.jsxs)(`label`,{className:`control-label`,children:[`最適化レベル`,(0,O.jsxs)(`span`,{className:`control-value`,children:[t,`%`]})]}),(0,O.jsx)(`input`,{type:`range`,min:`10`,max:`100`,value:t,onChange:e=>{n(Number(e.target.value)),c(!1)},className:`control-slider`,disabled:a})]}),(0,O.jsxs)(`div`,{className:`control-group`,children:[(0,O.jsxs)(`label`,{className:`control-label`,children:[`データ件数`,(0,O.jsxs)(`span`,{className:`control-value`,children:[r.toLocaleString(),`件`]})]}),(0,O.jsx)(`input`,{type:`range`,min:`1000`,max:`10000`,step:`500`,value:r,onChange:e=>{i(Number(e.target.value)),c(!1)},className:`control-slider`,disabled:a})]})]}),(0,O.jsxs)(`div`,{className:`control-actions`,children:[(0,O.jsxs)(`div`,{className:`toggle-group`,children:[(0,O.jsx)(`button`,{className:`toggle-btn ${d?`active`:``}`,onClick:()=>{f(!0),c(!1)},disabled:a,children:`量子`}),(0,O.jsx)(`button`,{className:`toggle-btn ${d?``:`active`}`,onClick:()=>{f(!1),c(!1)},disabled:a,children:`古典`})]}),(0,O.jsxs)(`button`,{className:`run-btn ${a?`running`:``}`,onClick:y,disabled:a,children:[(0,O.jsx)(ce,{size:13}),a?`実行中...`:`シミュレーション実行`]}),(0,O.jsxs)(`button`,{className:`reset-btn`,onClick:b,disabled:a,children:[(0,O.jsx)(le,{size:13}),`リセット`]})]})]}),(0,O.jsx)(`div`,{className:`metrics-grid`,children:e.metrics.map((e,t)=>(0,O.jsxs)(`div`,{className:`metric-card ${g} ${s?`metric-optimized`:``}`,children:[(0,O.jsxs)(`div`,{children:[(0,O.jsx)(`div`,{className:`metric-label`,children:e.label}),(0,O.jsx)(`div`,{className:`metric-value ${s?`metric-value-updated`:``}`,children:x(e.value,e.trend)}),s&&(0,O.jsx)(`div`,{className:`metric-badge`,children:`最適化済`})]}),(0,O.jsx)(`div`,{children:v(e.trend,!!(h||s))})]},t))}),(0,O.jsxs)(`div`,{className:`insights-panel`,children:[(0,O.jsxs)(`div`,{className:`insight-card impact-card ${g}`,children:[(0,O.jsxs)(`div`,{className:`card-header`,children:[(0,O.jsx)(de,{size:16,className:`insight-icon`}),(0,O.jsx)(`span`,{children:`経営インパクト (Business Impact)`})]}),(0,O.jsx)(`div`,{className:`card-body impact-text`,children:e.businessImpact})]}),(0,O.jsxs)(`div`,{className:`insight-card qvc-card ${g}`,children:[(0,O.jsxs)(`div`,{className:`card-header`,children:[(0,O.jsx)(he,{size:16,className:`insight-icon`}),(0,O.jsx)(`span`,{children:`量子 vs 古典 - 規模別比較`})]}),(0,O.jsx)(`div`,{className:`card-body`,children:(()=>{let t=Te(e.quantumVsClassical.classicalTime),n=Te(e.quantumVsClassical.quantumTime);return(0,O.jsxs)(`div`,{className:`qvc-scales`,children:[[{label:`小規模`,sublabel:`~1,000件`,cMult:.04,qMult:.18,classicalComment:`古典計算でも許容範囲内`,quantumComment:`量子優位性は限定的・コスト対効果を要検討`,verdict:`古典で対応可能`,verdictClass:`verdict-neutral`},{label:`中規模`,sublabel:`1,000~10,000件`,cMult:1,qMult:1,classicalComment:`処理時間が長くなり業務効率への影響が顕在化`,quantumComment:`量子優位性が明確に現れ始める規模`,verdict:`量子優位性が顕在化`,verdictClass:`verdict-quantum`},{label:`大規模`,sublabel:`10,000件超`,cMult:16,qMult:3.5,classicalComment:`実用的な時間内での処理はほぼ不可能`,quantumComment:`量子処理が事実上の必須要件`,verdict:`量子処理が必須`,verdictClass:`verdict-critical`}].map(r=>{let i=t==null?e.quantumVsClassical.classicalTime:Ee(t*r.cMult),a=n==null?e.quantumVsClassical.quantumTime:Ee(n*r.qMult),o=t!=null&&n!=null&&n*r.qMult>0?Math.round(t*r.cMult/(n*r.qMult)):null;return(0,O.jsxs)(`div`,{className:`qvc-scale-block`,children:[(0,O.jsxs)(`div`,{className:`qvc-scale-header`,children:[(0,O.jsx)(`span`,{className:`qvc-scale-label`,children:r.label}),(0,O.jsx)(`span`,{className:`qvc-scale-sublabel`,children:r.sublabel}),(0,O.jsx)(`span`,{className:`qvc-verdict ${r.verdictClass}`,children:r.verdict})]}),(0,O.jsxs)(`div`,{className:`qvc-scale-rows`,children:[(0,O.jsxs)(`div`,{className:`qvc-scale-row`,children:[(0,O.jsx)(`span`,{className:`qvc-scale-type classical-type`,children:`古典`}),(0,O.jsx)(`span`,{className:`qvc-time classical-time`,children:i}),(0,O.jsx)(`span`,{className:`qvc-scale-comment`,children:r.classicalComment})]}),(0,O.jsxs)(`div`,{className:`qvc-scale-row`,children:[(0,O.jsx)(`span`,{className:`qvc-scale-type quantum-type`,children:`量子`}),(0,O.jsx)(`span`,{className:`qvc-time quantum-time`,children:a}),(0,O.jsx)(`span`,{className:`qvc-scale-comment`,children:r.quantumComment})]}),o!=null&&o>1&&(0,O.jsxs)(`div`,{className:`qvc-speedup`,children:[`量子が `,(0,O.jsxs)(`strong`,{children:[o.toLocaleString(),`倍`]}),` 高速`]})]})]},r.label)}),(0,O.jsx)(`div`,{className:`advantage-text`,style:{marginTop:`8px`},children:e.quantumVsClassical.advantage})]})})()})]}),(0,O.jsxs)(`div`,{className:`insight-card verify-card ${g}`,children:[(0,O.jsxs)(`div`,{className:`card-header`,children:[(0,O.jsx)(ue,{size:16,className:`insight-icon`}),(0,O.jsx)(`span`,{children:`検証・信頼性サマリー (Safety & Compliance)`})]}),(0,O.jsx)(`div`,{className:`card-body verify-text`,children:e.verificationSummary})]})]})]})};function Oe(){let[e,t]=(0,_.useState)(D[0]),[n,r]=(0,_.useState)(``),[i,a]=(0,_.useState)(!1),[o,s]=(0,_.useState)(!1),[c,l]=(0,_.useState)([]);return(0,O.jsxs)(`div`,{className:`app-wrapper`,children:[(0,O.jsx)(`div`,{className:`top-bar`,children:(0,O.jsxs)(`a`,{href:`https://company-dashboard.up.railway.app/expo`,target:`_blank`,rel:`noopener noreferrer`,className:`portal-btn`,children:[(0,O.jsx)(se,{size:14}),`ポータルサイトへ戻る`]})}),(0,O.jsxs)(`div`,{className:`app-container`,children:[(0,O.jsxs)(`div`,{className:`pane glass-panel`,style:{flex:`0 0 400px`},children:[(0,O.jsxs)(`div`,{className:`pane-header`,children:[(0,O.jsx)(ie,{size:18,color:`var(--quantum-green)`}),(0,O.jsx)(`span`,{children:`エンタメ Vibe Coder`})]}),(0,O.jsx)(`div`,{className:`pane-content`,children:(0,O.jsx)(_e,{onGenerate:e=>{let n=D.find(t=>t.id===e)||D[0];l(e=>[...e,{role:`user`,content:n.prompt}]),a(!0),s(!1),r(``),setTimeout(()=>{r(n.codeSnippet),t(n),a(!1),l(e=>[...e,{role:`assistant`,content:`Quantum Execution Completed: 予測モデルの構築に成功しました。`}])},2800)},onReset:()=>{l([]),r(``),s(!1),a(!1)},isGenerating:i,history:c})})]}),(0,O.jsxs)(`div`,{className:`pane glass-panel`,style:{flex:`1.2`},children:[(0,O.jsxs)(`div`,{className:`pane-header`,children:[(0,O.jsx)(ae,{size:18,color:`var(--quantum-blue)`}),(0,O.jsx)(`span`,{children:`quantum_entertainment_engine.py`})]}),(0,O.jsx)(`div`,{className:`pane-content`,children:(0,O.jsx)(ve,{code:n,isGenerating:i,onAnimationComplete:()=>s(!0)})})]}),(0,O.jsxs)(`div`,{className:`pane glass-panel`,style:{flex:`0 0 500px`},children:[(0,O.jsxs)(`div`,{className:`pane-header`,children:[(0,O.jsx)(oe,{size:18,color:`#eab308`}),(0,O.jsx)(`span`,{children:`ライブダッシュボード (エンタメ Control Center)`})]}),(0,O.jsx)(`div`,{className:`pane-content`,style:{padding:`16px`},children:o?(0,O.jsx)(De,{activeUseCase:e}):(0,O.jsxs)(`div`,{style:{display:`flex`,flexDirection:`column`,alignItems:`center`,justifyContent:`center`,height:`100%`,gap:`16px`,opacity:.45},children:[(0,O.jsx)(oe,{size:48,color:`#eab308`,style:{animation:i?`pulse 1.2s infinite`:`none`}}),(0,O.jsx)(`p`,{style:{fontSize:`0.85rem`,textAlign:`center`,color:`var(--text-muted)`,lineHeight:1.6},children:i?`Quantum tensors resolving…
ダッシュボードを構築中`:`ユースケースを選択して
「Vibe Coding」を押してください`})]})})]})]})]})}(0,v.createRoot)(document.getElementById(`root`)).render((0,O.jsx)(_.StrictMode,{children:(0,O.jsx)(Oe,{})}));